...
```
+-- SESSION TREE (SC-07, 좌측 세로 트리) ----+
| v tower2                                   |  <- 탭(부모)
|   [x] tower2 [>]c (PID 1234)               |  <- 터미널(자식): [ ]주입대상 체크 + 표시명 + PID
|   [ ] proj        (PID 5678)               |
| v sangmin [>]                              |
|   [x] sangmin (PID 4321)                   |
+--------------------------------------------+
```
캡션: 좌측 **세로 TreeView**로 탭(부모)→터미널(자식) 전환(FR-006, v1 실구현). 선택 노드가 우측 pane 포커스(파란 테두리)와 동기하며, 자식 노드 앞 체크박스로 다중 주입 대상을 지정한다(SC-12 연계). `c`=claude 자동실행, 배지로 상태 식별.
...
- 목적: 대상 세션 입력 파이프에 커맨드 전송. v1 실구현은 좌측 트리 컬럼 하단에 상주하며, 주입 대상은 트리 자식 노드의 체크박스 또는 현재 선택 터미널로 결정한다.
- 핵심 구성요소: 커맨드 입력 TextBox / [실행] 버튼 / 대상 = 트리에서 체크된 터미널(IsInjectTarget) 있으면 그 전체, 없으면 현재 선택 터미널.
- 표시 데이터: {commandText}, {checkedTargets[] | selectedTerminal}.
- 상호작용: 입력 후 Enter 또는 [실행]→대상(들) stdin에 독립 write(개행 부착, NFR-002).
- 상태: 빈=커맨드 없음→[실행] 무동작 / 에러=대상 세션 미준비 시 write 무시.
- 진입: 좌측 트리 컬럼 하단 상시 / 이탈: 전송→SC-08 반영.
```
+-- COMMAND BAR (SC-12, 좌측 트리 컬럼 하단) --+
| [ {cmd}__________ ]              [ 실행 ]     |
+----------------------------------------------+
  대상: 트리에서 [x]체크된 터미널(다중) 또는 선택 터미널
```
캡션: 대상은 좌측 트리 자식 노드의 체크박스(다중) 또는 현재 선택 터미널로 결정된다(v1 실구현). 설계상의 대상 표시 칩·single/multi 토글·위험 게이트(SC-13) 연동은 상위 설계 로드맵으로 남긴다.
...
- 분류/존: Main · 세션 뷰 하단(전폭 행) · 대상 UT-002·003
- FR/FN: FR-047 / FN-SES-11 (관련 NFR-002)
- 목적: A 세션 출력을 캡처 버퍼에 수집→사용자 뷰·편집(가공)→대상 세션 입력으로 주입. 세션 간 raw 출력 라우팅(IPC 파일 채널과 별개).
- 핵심 구성요소: 출처 표시(from {SourceAs}) / [캡처] 버튼(선택 터미널 현재 출력 수집) / 편집 가능한 버퍼 TextBox / [대상에 주입] 버튼.
- 표시 데이터: {SourceAs, bufferContent(편집)}, {checkedTargets[] | selectedTerminal}.
- 상호작용: [캡처]→선택 터미널 출력을 버퍼 적재 / 버퍼 편집 / [대상에 주입]→트리에서 체크된 터미널(다중) 없으면 선택 터미널에 write.
- 상태: 빈=버퍼 비어있음 / 에러=대상 미준비 시 write 무시.
- 진입: 세션 뷰 하단 상시 / 이탈: 주입 후 대상 세션 반영.
```
+-- 출력 캡처 버퍼 (SC-24, 세션 뷰 하단 전폭) --------+
| 출력 캡처 버퍼  (from tower2)   [캡처] [대상에 주입] |
| {editable buffer content.......................}   |
+----------------------------------------------------+
  대상: 트리에서 [x]체크된 터미널(다중) 또는 선택 터미널
```
캡션: A 출력을 버퍼로 받아 편집 후 대상에 주입 — 클립보드 수동 복붙(FN-TRM-13)의 오케스트레이션 승격(v1 실구현). 설계상의 우측 R존 배치·single/multi 선택기·captured_at은 상위 설계 로드맵으로 남긴다.
