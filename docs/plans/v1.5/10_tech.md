...
> 담당: plan_tech_researcher · 깊이: deep · 스택 9영역 / RISK 10 / NFR 충족 20/20
...
본 문서는 Control Tower v1의 기술 스펙 단일 원천이다. `04_requirements`(FR 45/NFR 20·제약)·`07_interfaces`(SC 23)·도메인 모델을 입력으로, "무엇을·왜"를 "어떤 기술로·어떻게"로 확정한다.

- **정의하는 것**: (a) 영역별 기술 스택과 근거·대안(§3) / (b) Feature×Layer 아키텍처(§4) / (c) 대표 데이터 흐름(§5) / (d) 외부 라이브러리·native 의존(§6) / (e) 기술 리스크 RISK-### + PoC(§7) / (f) 배포 토폴로지·비용(§8) / (g) NFR 20종 전수 충족 검증(§9).
...
- **핵심 제약**: C1(앱-소유 세션만 제어) · C2(터미널 substrate가 0순위 — 모든 오케스트레이션이 세션 I/O 소유에 의존) · C4(IPC는 skill_ipc_control 재사용, 재구현 금지) · C7(로컬·인증 없음) · C8(ClickOnce) · C9(Feature×Layer).
...
| **실시간 요구** | (a) PTY 출력 스트림 실시간 렌더(공식 WT GPU 렌더러가 흡수) · (b) IPC inbox.log watch ≤2s | NFR-001·004 |
| **데이터 특성** | 저volume 정형(프로파일 N개·JSON) + 고volume 스트림(PTY 바이트). 읽기≫쓰기 | FR-020 |
...
| **특수 기술** | (1) 임베드 터미널 컨트롤(공식 WT 백엔드) · (2) native 자산 배포(conpty.dll·OpenConsole.exe) · (3) 프로그램적 주입·출력 캡처 · (4) 파일 watch/원자 write | NFR-011 |
...
- 근거: 출력 캡처는 진단·자동반응용 보조 관측 채널로 쓴다.
...
- 추천: 데이터량 적고 레코드 수도 적어서 JSON + 원자적 temp→rename. NFR-011 = `File.WriteAllText(temp)`→`File.Move(temp,dest,overwrite:true)`. 프로파일 수·쿼리 요구 증가 시 SQLite 승격(최종 스키마·ERD는 09).
...
|       |     +------+-----------+                 |              |          |
|  +----v---------+  | spawn N   +-----------------v---+   +------v-------+  |
|  | Storage JSON |  |           | Ipc file contract   |   | Asset tree   |  |
|  | (atomic)     |  |           | (read/watch)        |   | (read/write) |  |
|  +--------------+  |           +---------------------+   +------+-------+  |
|            +-------v-----------------------+                    |          |
...
|          +--+---------------------+--------+                    |          |
+-------------|-----------------|----|----------------------------|----------+
      conpty.dll |    OpenConsole.exe |                  (FS read/write) |
         +-------v-----------------v--+              +-------------v-------+
         |  ConPTY (in/out pipes)     |              | channels/<ch>/      |
         +-------------+--------------+              | ~/.claude/{rules,   |
                       |                             |  skills,agents}     |
                 +-----v-----+  ... (app-owned       +---------------------+
                 |  pwsh     |      processes only, C1)
                 +-----------+
```
캡션: EmbeddedTerminal Feature가 EasyTerminalControl(공식 WT 백엔드)을 호스팅하고, 소유 경계 `ITerminalSession`이 주입/캡처/수명을 감싼다(엔진↔폴백 교체를 이 1곳에서, NFR-018). 컨트롤 내부에서 conpty.dll·OpenConsole.exe가 pwsh를 spawn하고 Microsoft.Terminal.Control이 렌더한다 — VT 파싱·셀 렌더는 컨트롤이 담당. SessionManager는 앱-소유 세션만 추적한다(C1). IPC/자산은 파일시스템만 read/watch(+write-through)한다(C4).
...
sequenceDiagram
    ...
    participant PW as pwsh
    participant OB as Diagnostics(capture)
...
### 5-3. IPC 채널 watch (FR-024 · NFR-004)
```mermaid
sequenceDiagram
...
| EXT-06 | System.Text.Json | 프로파일 영속(JSON 직렬화) | FR-020 | MIT(내장) | 무료 | 낮음 | Newtonsoft |
...
- **기준 하드웨어(성능 게이트 판정 기준)**: 합리적 Win11 x64 데스크톱 — **4코어 CPU · 16GB RAM · SSD**. NFR-003(동시 세션 ≥8)·성능 게이트(11 G7)·성능 NFR(001 렌더·002 주입·004 watch) 판정은 이 기준 사양에서 측정한다(저사양은 best-effort).
...
> **충족률 20/20 (100%), 미매핑 0.** 위반 발생 시 §3 스택 보강(또는 폴백 TS-03)으로 복귀.

---

## 10. 요약

- **영역별 추천 스택**: WPF(.NET10 LTS)+MVVM · **EasyWindowsTerminalControl(INTEGRATE/BUY, MIT) — 공식 WT 렌더러+ConPTY+VT 일체** · TermPTY 주입/캡처 · self-build는 폴백(TS-03) · JSON 원자 저장(09) · skill_ipc_control 소비(REUSE) · ClickOnce+native 동봉 · 인증 없음(포트 0).
...
- **외부 의존**: 코드 의존 11건(EXT-01~12), 외부 네트워크 SaaS·유료 API 0. 핵심 결합 = 엔진(EXT-01)+native(EXT-02/03)·IPC 계약(EXT-08).
- **RISK 총수 10** — ★최상 1(RISK-001 native 배포).
- **예상 비용**: ~$0~$40/월(서명 인증서 외 사실상 0, 엔진 MIT).
- **NFR 충족률 20/20 (100%)** — 미매핑 0.
