...
> 담당: orchestrator(합성) · skill_plan v1 · 13문서 · 2026-07-01

---

## 한 줄 정의

**Control Tower** — 로컬에서 여러 Claude Code/PowerShell 세션을 **앱이 직접 소유(ConPTY 임베드)** 하여, 의도별 **세션 프로파일**로 띄우고 · **IPC 채널**로 묶어 협업시키며 · **프롬프트 자산**까지 한 곳에서 관제하는 **단일 파워유저용 AI 세션 관제탑** (WPF/.NET 10 데스크톱).

## 핵심 컨셉 (4축)

① ConPTY 앱-소유 터미널(substrate) · ② 세션 프로파일 한 벌 · ③ IPC 오케스트레이션(skill_ipc_control 프론트엔드) · ④ 프롬프트 자산 편집(~/.claude). **차별축**: 세션 간 IPC 협업 — 경쟁(worktree 격리)에 부재.
...
| # | 문서 | 내용 | 담당 | 핵심 지표 |
|---|---|---|---|---|
| 01 | [`01_overview`](./01_overview.md) | 제품 개요·정체성·핵심컨셉·범위·성공기준 | 합성 | 4축·9제약 |
...
| 04 | [`04_requirements`](./04_requirements.md) | 요구사항 FR/NFR·7카테고리·제약 | requirement_analyzer | **FR 45 · NFR 20** |
| 05 | [`05_functions`](./05_functions.md) | 기능정의 FN·추적성 매트릭스 | function_specifier | **FN 55** (FR 전수 커버) |
| 06 | [`06_behaviors`](./06_behaviors.md) | 행동 시나리오·Journey Map | behavior_designer | BS 22 · JM 5 · 엣지 14 |
| 07 | [`07_interfaces`](./07_interfaces.md) | IA·화면 SC·흐름·UX | interface_designer | **SC 23** (FR 커버 45/45) |
| 08 | — (제외) | REST/DTO — 로컬 데스크톱, 서버 API 없음 | (excluded) | in-proc 계약은 10 |
| 09 | [`09_database`](./09_database.md) | 데이터 모델·ERD·ENT·영속화 | db_modeler | **ENT 17** (영속 8/투영 4/런타임 5) |
| 10 | [`10_tech`](./10_tech.md) | 기술 스택·아키텍처·리스크·PoC | tech_researcher | **RISK 10** · NFR 20/20 |
| 11 | [`11_test`](./11_test.md) | 테스트·QA 전략·품질 게이트 | test_strategist | TC 123 · 게이트 9 |
| 12 | [`12_roadmap`](./12_roadmap.md) | 실행 로드맵·레이어·일정 | roadmap_planner | 5단계 · FR 매핑 45/45 |
| 13 | [`13_followups`](./13_followups.md) | 후속 숙제·운영 사전확인 | 합성 | 숙제 8 · 운영 5 |

> **08 제외 근거**: 본 제품은 로컬 데스크톱으로 REST 백엔드가 없다(수신 포트 0). 통합 계약(ConPTY interop·IPC 파일)은 10 tech가 다룬다.

---

## 권장 읽기 순서

1. **개요 파악**: `01_overview` → `INDEX`(본 문서)
...
- **식별자 총계**: FR 45 · NFR 20 · FN 55 · UT 5 · P 1 · BS 22 · JM 5 · SC 23 · ENT 17 · RISK 10 · 공통 카테고리 7(TRM·SES·PRF·IPC·AST·SEC·SYS)
- **MoSCoW(FR)**: Must 28 · Should 16 · Could 1
- **레이어 로드맵**: P0(압축 PoC) → L0(터미널) → L1(세션·프로파일=MVP) → L2(IPC) → L3(자산) + 횡단(SEC·SYS)
- **핵심 결정**: **EasyWindowsTerminalControl(공식 WT 렌더러 임베드, INTEGRATE/BUY, MIT)** · self-build는 폴백 · JSON-first 영속 · IPC 재사용(REUSE)
- **무결성**: FR 전수 커버리지 FN/SC/ENT/로드맵 **45/45** · 고아 참조 0 · 중복 ID 0 (v1.1 델타: FR-046·047·048)
...
| FR-010 | TRM-13 | 09 | 001 |
| FR-044 | TRM-14 | 10 | 003 |
| FR-045 | TRM-15 | 07·08 | 004 |
| FR-046 | TRM-16 | 07·23 | 003·004 |
...
> **매트릭스 무결성**: 45개 FR 전부가 ≥1 FN·≥1 SC·≥1 ENT로 매핑됨(미커버 0). NFR-only 실현 기능 FN-SEC-04(NFR-008)·FN-SYS-05(NFR-022)는 FR 소급이 아니라 별도(05 §11-3).
