...
| 5 | VT 렌더 | 공식 WT 렌더러가 전체 VT(색·커서·스크롤백·alt-screen/TUI) 담당 | 자체 단계화 불요 |
| 6 | 구현 전략 | **EasyWindowsTerminalControl(공식 WT 렌더러 임베드)** 채택 | 파서·렌더러 자체제작 대공사 회피. self-build는 폴백 |
...
### 핵심 의존 함의 (입력 경로 ↔ 오케스트레이션)

```
IPC 스킬 주입 / 커맨드 전송  ── 입력 경로만 필요 ──▶  터미널 렌더와 무관하게 동작 (주입 경로 독립)
claude 화면 관찰 / TUI 관제  ── 공식 WT 렌더러 ────▶  기본 제공 (alt-screen TUI 내장 렌더)
```

주입/제어 경로는 입력 경로만으로 확보되어 터미널 렌더 완성도와 무관하다. 이 분리 덕에
레이어 2 오케스트레이션이 터미널 렌더에 볼모잡히지 않는다.
...
```
레이어 0 ★ 임베드 터미널      앱-소유 pwsh 세션 · EasyWindowsTerminalControl(공식 WT 렌더러) · claude TUI 06 GO · 단일 pane
