...
│   ├── services.md              # 프로세스 제어 로직·인터페이스 분리
...
| [services.md](./convention/services.md) | 프로세스 추적·실행, GC 방지 강한 참조, 인터페이스 |
...
| ViewModels | `Features/<F>/ViewModels/` | `...Features.<F>.ViewModels` | ViewModelBase 상속, Service 주입 |
| Services | `Features/<F>/Services/` | `...Features.<F>.Services` | UI 무지향, GC 방지 강한 참조 |
...
- [ ] 파일 1개뿐인데 레이어 폴더를 강제하지 않았는가(점진적 승격)
