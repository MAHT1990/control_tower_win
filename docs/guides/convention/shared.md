...
│   └── Interfaces/   (공유 계약 — 예: IDialogService.cs)   # 항상 분리
...
네임스페이스: `ControlTowerWin.Shared.Core`, `ControlTowerWin.Shared.Core.Interfaces`, `ControlTowerWin.Shared.Converters` 등.

> **공유 계약도 분리**: `Shared/Core`의 인터페이스(`I*`)는 `Shared/Core/Interfaces/`에 둔다
> (Feature와 동일하게 항상 분리). 상세는 [interfaces.md](./interfaces.md).
